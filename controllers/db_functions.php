...
// FUNCION PARA CHEQUEAR SI ya EXISTE EN LA BD EL NOMBRE DE USUARIO INGRESADO EN EL FORM DE REGISTRO 
...
function modificar_usuario(
    ...
    try {
        ...
        if (!$stmt1) {
            throw new Exception("Error en prepare(): " . $mysqli_connection->error);
        }
        ...
        if (!$stmt1->execute()) {
            throw new Exception("Error en execute(): " . $stmt1->error);
        }
        ...
        if (!$stmt2->execute()) {
            throw new Exception("Error en execute(): " . $stmt2->error);
        }
        $stmt2->close();
...
function borrar_usuario(int $idUser, mysqli $db): bool
{
    try {
        ...
        $stmt2 = $db->prepare("DELETE FROM users_data WHERE idUser = ?");
...
// Validar DATOS ingresados en CREAR CITA (USER y ADMIN)
...
/* --------------------------------------------------
    ...
    -------------------------------------------------- */
function disponibilidad_cita($fecha, $hora, $mysqli_connection)
{
    ...
    if (!$stmt->execute()) {
        throw new Exception("Error al ejectutar consulta de disponibilidad: " . $stmt->error);
    }
...
function crear_cita(
    ...
    if (!$stmt) {
        throw new Exception("Error al preparar la inserción de la cita: " . $mysqli_connection->error);
    }

    $stmt->bind_param("isss", $idUser, $fecha, $hora, $motivo);

    if (!$stmt->execute()) {
        throw new Exception("Error al ejectutar la inserción de la cita: " . $stmt->error);
    }
...
function actualizar_cita(
    ...
    if (!$stmt) {
        throw new Exception("Error al preparar la actualización de la cita: " . $mysqli_connection->error);
    }

    $stmt->bind_param("sssi", $fecha, $hora, $motivo, $idCita);

    if (!$stmt->execute()) {
        throw new Exception("Error al ejecutar la actualización de la cita: " . $stmt->error);
    }
...
/* ==========================
   BORRAR CITA
========================== */
function borrar_cita(int $idCita, mysqli $mysqli_connection): void
{
    ...
    if (!$stmt) {
        throw new Exception("Error al preparar el borrado de la cita: " . $mysqli_connection->error);
    }

    $stmt->bind_param("i", $idCita);

    if (!$stmt->execute()) {
        throw new Exception("Error al ejecutar el borrado de la cita: " . $stmt->error);
    }
...
/* ======================================================
   OBTENER TODAS LAS CITAS VIGENTES DE TODOS LOS USUARIOS
========================================================= */
function obtener_citas_vigentes($db): array
{
    $query = "
        ...
        WHERE TIMESTAMP(c.fecha_cita, c.hora_cita) >= NOW()
    ...
    if (!$result) {
        throw new Exception("Error obteniendo citas vigentes: " . $db->error);
    }

    return $result->fetch_all(MYSQLI_ASSOC);
}
/* ======================================================
   OBTENER TODAS LAS CITAS PASADAS DE TODOS LOS USUARIOS
========================================================= */

// Función para obtener las citas pasadas

function obtener_citas_pasadas($db): array
{
    $query = "
        SELECT c.idCita, c.fecha_cita, c.hora_cita, c.motivo_cita,
               u.idUser, u.nombre, u.apellidos
    ...
    if (!$result) {
        throw new Exception("Error obteniendo citas pasadas: " . $db->error);
    }
...
/** USER**/
/* ======================================================
   OBTENER TODAS LAS CITAS VIGENTES DEL USUARIO logueado
========================================================= */
function obtener_citas_vigentes_user($id_user, $db): array
{
    $query = "
        SELECT c.idCita, c.fecha_cita, c.hora_cita, c.motivo_cita
        FROM citas c
        WHERE c.idUser = ?
        AND TIMESTAMP(c.fecha_cita, c.hora_cita) >= NOW()
        ORDER BY c.fecha_cita, c.hora_cita
    ";
...
/* ======================================================
   OBTENER TODAS LAS CITAS PASADAS DEL USUARIO LOGUEADO
========================================================= */

// Función para obtener las citas pasadas

function obtener_citas_pasadas_user($id_user, $db) {
    ...
    $result = $stmt->get_result();
    $pastCitas = $result->fetch_all(MYSQLI_ASSOC);
